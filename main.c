...
// assuming (k * world_size) is the number of documents in the input
		...
		for(i = 0; i < k && change == 1; i++){
			...
			for(j = 0; j < world_size; j++){
				...
				if(count < k){
					...
					leastid[count] = leastkids[j][i];
					leastval[count] = leastkvals[j][i];
					...
					// insertion_sort swaps the id's according to the values as well
					if(leastval[k-1] > leastkvals[j][i]){
						leastval[k-1] = leastkvals[j][i];
						leastid[k-1] = leastkids[j][i];
						change = 1;
					}
					insertion_sort(leastval, leastid, k);
					// int a;
